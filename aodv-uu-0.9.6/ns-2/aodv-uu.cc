...
void NS_CLASS packetFailed(Packet *p)
{
	...
	fprintf(stderr,"local repair status is %d \n",local_repair);
	/* Do local repair? */
	/* Rewrite by BUAA_Yxy */
	if (local_repair)
	{
        /* Buffer the current packet with a now data queue PQ_RPR */
        fprintf(stderr,"-------------start buffering the packet--------\n");
        packet_queue_rpr_add(p, dest_addr);
        ...
        rrpr_local_repair(rt, src_addr, NULL);
	}

    /* End rewrite by BUAA_Yxy */
	if (local_repair && rt->hcnt <= MAX_REPAIR_TTL // #define MAX_REPAIR_TTL          3 * NET_DIAMETER / 10
