...
/** func added by BUAA_Yxy begin **/
void NS_CLASS packet_queue_rpr_init(void)
{
    INIT_LIST_HEAD(&PQ_RPR.head);
    PQ_RPR.len = 0;

#ifdef GARBAGE_COLLECT
    /* Set up garbage collector */
    timer_init(&PQ_RPR.garbage_collect_timer, &NS_CLASS packet_queue_a_timeout, &PQ_RPR);

    timer_set_timeout(&PQ_RPR.garbage_collect_timer, GARBAGE_COLLECT_TIME);
#endif
}


void NS_CLASS packet_queue_rpr_destroy(void)
{
    int count = 0;
    list_t *pos, *tmp;

    list_foreach_safe(pos, tmp, &PQ_RPR.head) {
        struct q_pkt *qp = (struct q_pkt *)pos;
        list_detach(pos);
...
/* Garbage collect packets which have been queued for too long... */
int NS_CLASS packet_queue_garbage_rpr_collect(void)
{
    int count = 0;
    list_t *pos, *tmp;
    struct timeval now;
...
void NS_CLASS packet_queue_rpr_add(Packet * p, struct in_addr dest_addr)
{
    struct q_pkt *qp;
    struct hdr_ip *ih;
    struct hdr_cmn *ch = HDR_CMN(p);
    ...
    if (PQ_RPR.len >= MAX_QUEUE_LENGTH) {
        DEBUG(LOG_DEBUG, 0, "MAX Queue length! Removing first packet.");
        if (!list_empty(&PQ_RPR.head)) {
            qp = (struct q_pkt *)PQ_RPR.head.next;

            list_detach(PQ_RPR.head.next);
    ...
    list_add_tail(&PQ_RPR.head, &qp->l);

    PQ_RPR.len++;

    DEBUG(LOG_INFO, 0, "buffered_a pkt to %s uid=%d qlen=%u",
          ip_to_str(dest_addr), ch->uid(), PQ_RPR.len);
}

int NS_CLASS packet_queue_set_rpr_verdict(struct in_addr dest_addr, int verdict)
{
    int count = 0;
    rt_table_t *rt, *next_hop_rt, *inet_rt = NULL;
    list_t *pos, *tmp;
...
#define ARP_DELAY 0.005
    ...
    if (rt && rt->state == VALID &&
        ...
        DEBUG(LOG_INFO, 0, "SENT %d packets_A to %s qlen=%u",
              count, ip_to_str(dest_addr), PQ_RPR.len);
    } else if (verdict == PQ_DROP) {
